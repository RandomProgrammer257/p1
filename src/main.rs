...
static G: f32 = 6.67 * 0.00000000001;
...
fn planet_prepare(
    mut planet_query: Query<( &mut PlanetPreGravity,&mut AdditionalMassProperties,&PlanetDensity,&PlanetVolume), With<Planet>,>,
) {
...
fn world_gravity_sistem(
    ...
    for (transform, mut external_force) in &mut player_query {
        ...
        for(planet_pre_gravity, external_force_planet, transform_planet) in &mut planet_query{
                let range = ((transform_planet.translation.x - transform.translation.x).powf(2.0) +  (transform_planet.translation.y - transform.translation.y).powf(2.0)).powf(0.5);

                full_ext_forse.0 += planet_pre_gravity.0/range.powf(3.0) * (transform_planet.translation.x - transform.translation.x);
                full_ext_forse.1 += planet_pre_gravity.0/range.powf(3.0) * (transform_planet.translation.y - transform.translation.y);
