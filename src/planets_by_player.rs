//! Модуль спавна планет игроком
//!
//! Содержит логику создания планет по клику мыши с учетом
//! гравитационного влияния ближайших объектов.
...
/// Поиск ближайшего по гравитационному воздействию объекта
///
/// Вычисляет, какой из объектов (планет или звезд) оказывает наибольшее
/// гравитационное влияние на точку спавна.
///
/// # Аргументы
/// * `spawn_point` - точка спавна в мировых координатах `Vec2`
/// * `spawn_radius` - радиус создаваемой планеты
/// * `spawn_density` - плотность создаваемой планеты
/// * `spawn_zone_size` - размер зоны гравитации создаваемой планеты
/// * `planets_query` - запрос ко всем планетам
/// * `star_query` - запрос ко всем звездам
///
/// # Возвращает
/// * `Option<Entity>` - сущность ближайшего объекта или `None`, если объектов нет
fn get_nearest(
    spawn_point: Vec2,
    spawn_radius: f32,
    spawn_density: f32,
...
/// Обработчик спавна планеты по клику мыши
///
/// Система, которая создает новую планету в позиции курсора при клике
/// левой кнопкой мыши, если включен режим спавна планет.
///
/// # Особенности
/// * Скорость планеты вычисляется как орбитальная скорость относительно ближайшего гравитационного объекта
/// * Начальная скорость направлена перпендикулярно радиус-вектору к ближайшему объекту
/// * Если ближайший объект не найден, планета создается с нулевой скоростью
///
/// # Режим работы
/// * Спавн происходит только при активном `planet_spawn_mode` в `MouseStates`
/// * Позиция спавна берется из `world_position` ресурса `MouseStates`
/// * Размер планеты зависит от масштаба камеры (`Campos`)
